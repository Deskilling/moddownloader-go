...
func profileLastVersionId(loader string, version string) string {
	if loader == "fabric" {
		return fmt.Sprintf("fabric-loader-%s-%s", getLatestFabricVersion(), version)
	} else if loader == "quilt" {
		return fmt.Sprintf("quilt-loader-%s-%s", getLatestQuiltVersion(), version)
	} else if loader == "forge" {
		return fmt.Sprintf("%s-forge-%s", version, getLatestForgeVersion(version))
	}
...
func exportModpack(modpackPath string, launcherPath string) {
	...
	modpackPath, err := checkMrpack(modpackPath)
	if err != nil {
	...
	err = extractZip(modpackPath, "temp/")
	if err != nil {
	...
	fmt.Println("Copyy ")
	copyDirectory("temp/", launcherPath)
